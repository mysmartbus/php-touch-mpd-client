    ...
    private function _updatePlaylists($arg1, $songuri) {
        ...
        **/
        ...
        foreach($lists as $key => $playlist) {
            ...
            // Remove these elements from the $contents array
            $removeme = array();

            // Is the song on $playlist?
            $found = false;

            // Loop through songs on playlist to determine if
            // the $songuri needs to be added or removed.
            foreach ($contents as $k => $pluri) {
                if ($songuri == $pluri) {

                    // Song is on $playlist
                    $found = true;

                    if (!in_array($playlist, $assignedplaylists)) {
                        // Keep track of each time $songuri was found in $contents
                        // unless $playlist is in $assignedplaylists
                        $removeme[] = $k;
                    }
                }
            }
            ...
            if (($found === false) && in_array($playlist, $assignedplaylists)) {
                // Add song to playlist ONLY if the song was not found on $playlist
                // AND $playlist is in $assignedplaylists.
                $rv = $this->_sendCommand('playlistadd', $playlist, $songuri);
            }

            if (!empty($removeme)) {
                // Need to remove the song from $playlist

                // Sorts the array by value from lowest to highest
                // Array index is reset
                sort($removeme);

                $loopcnt = 0;

                foreach($removeme as $k => $index) {

                    // array_splice() does not preserve numeric keys.
                    // Need to correct $k so it is in the correct position.
                    $k -= $loopcnt;
