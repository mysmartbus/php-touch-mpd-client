        ...
        **/
        ...
        foreach($lists as $key => $playlist) {
            ...
            foreach ($contents as $k => $uri) {
                if ($songuri == $uri && !in_array($playlist, $assignedplaylists)) {
                    // Set the flag to remove the song from the playlist
                    $found = true;
                    break;
                }
            }
            ...
            if ($found === true) {
                ...
                echo 'removed from '.$playlist.'<br>';
            }

            // Add song to playlist ONLY if it was not found in $playlist and
            // $playlist is in the array $assignedplaylists.
            if ($found === false && in_array($playlist, $assignedplaylists)) {
                $rv = $this->_sendCommand('playlistadd', $playlist, $songuri);
                echo 'added to '.$playlist.'<br>';
            }
       }
